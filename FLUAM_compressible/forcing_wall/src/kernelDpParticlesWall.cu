...
__global__ void kernelDpParticlesWall(double* densityGPU, 
  ...
  /******** Adolfo. Aqui es donde se cambia la fuerza de las celdas ***/
  /**** Added by Adolfo ****/
  // int aa = (i % (mx*my)) / mx;
  /******************/
  //Particle force
...
__global__ void kernelDpParticlesWall_shearRate(double* densityGPU,
					double* vxGPU, 
					double* vyGPU,
					double* vzGPU,
					double* dmGPU,
  ...
  /***** Note that the calculation of dydvxGPU is valid only if the cell is the first one,
  ...
  /******** Next is gradv in y direction *****/
  int vecino44;
  // 2 celdas mas arriba en la direccion y
  //  vecino44 = ghostIndex[ realIndex[i + 2 * mxtGPU] ];  
  vecino44 = tex1Dfetch(texvecino4GPU,vecino4);
  // 3 celdas mas arriba en la direccion y
  // vecino444 = tex1Dfetch(texvecino4GPU,vecino44);
  double vx = fetch_double(texVxGPU,i);
  double vx4 = fetch_double(texVxGPU,vecino4);
  double vx44 = fetch_double(texVxGPU,vecino44);
  // double vx444 = fetch_double(texVxGPU,vecino444);

  // *** Calculado solo con 2 celdas ***
  double vx1 = fetch_double(texVxGPU,vecino1);

  /*** Suponemos que la y de Vx esta en mitad de la celda, y que Vwall 
      esta en el limite inferior de la primera celda (o el limite superior de la celda 0) ***/
  // --- Third order derivative --- *** El gradiente de la velocidad en la pared se almacena correctamente solo en las celdas en las que fy = 1 ***
  // dydvxGPU[i] = invdxGPU * (15.0/4.0 * vx -5.0/6.0 * vx4 + 3.0/20.0 * vx44 - 46.0/15.0 * vxWall );
  dydvxGPU[i] = 2.0 * invdxGPU * (vx - vxWall);
...
/****** Subroutine added by Adolfo *******/
__global__ void kernelDpParticlesWall_stresswall(double* densityGPU, 
				      double* densityGPU2,
				      double* vxGPU, 
				      double* vyGPU,
				      double* vzGPU,
  ...
  /**** Adolfo: aqui esta el estres de las celdas ****/
  /*** El stress en la pared es sXY (cuando ay == 0) ***/
  /**** Mirar si el ruido tambien se aplica a las de pared ***/
  // int ax = i % mxGPU;
  // int az = i / (mxGPU*myGPU);
  // int ay = (i % (mxGPU*myGPU)) / mxGPU;
  //if (ay == 0)
  //  sXYGPU[i] = sXY;
  //    sXYGPU[i] = invdyGPU * (vx4 - vx);
  sXYGPU[i] = invdyGPU * (vx - vx1);
  //  sXYGPU[i] = -shearviscosityGPU * (invdyGPU * (vx4 - vx) + invdxGPU * (vy3 - vy));
  // sXYGPU[i] += shearviscosityGPU * (invdyGPU * (vx - vx1) + invdxGPU * (vypxmy - vy1));
  //  if (ay == 0)
  //	printf("eeee %i %f %f\n", i , sXY, sYX); 
    //   printf("eeee %i %i %i %f %f\n", ax , ay, az, sXY, sYX); 
  // printf("eeee %i %i %i %f %f %f\n",ax, ay, az, vx, vy, vz); 
