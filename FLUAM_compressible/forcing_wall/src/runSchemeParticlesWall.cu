...
bool runSchemeParticlesWall(){
  ...
  *counter = 0;
  ...
  while(step<numsteps){
    ...
    /** About the shear stress on the wall ***/
    cutilSafeCall(cudaMemcpy(csXY,sXYGPU,ncellst*sizeof(double),cudaMemcpyDeviceToHost));
    ...
    // visc_force before here is the summation of the shear rate along all the cells.
    ...
    */
    ...
    /* Forcing wall */
    double Force_total = visc_force + polymer_force_wall + F0*cos(freq*time) - k_wall * xWall;

    // cout << visc_force << " " << polymer_force_wall << endl;

    /* The next was for the ring-down (or to enforce the force on the wall) */
	...
	/*	cout << "force " << time << " " << Force_total << endl; */
        cout << "force " << time << " " << visc_force << " " << polymer_force_wall << " " << F0*cos(freq*time) << " " <<  - k_wall * xWall << " " << Force_total << endl;
      }
    ...
    */
    ...
    kernelDpParticlesWall_stresswall<<<numBlocksCellst,threadsPerBlockCellst>>>(densityGPU,
										densityGPU,
										vxGPU,
										vyGPU,
										vzGPU,
    ...
    */
    ...
    /*kernelDpParticlesWall<<<numBlocksCellst,threadsPerBlockCellst>>>(densityPredictionGPU,
								     ...
								     0.75,0.25,sqrt(3));
    */
    kernelDpParticlesWall_stresswall<<<numBlocksCellst,threadsPerBlockCellst>>>(densityPredictionGPU,
										densityGPU,
										vxGPU,
										vyGPU,
										vzGPU,
