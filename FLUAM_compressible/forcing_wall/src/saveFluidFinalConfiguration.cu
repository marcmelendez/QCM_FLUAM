...
/******** Added by Adolfo **********/
bool saveFluid_sXY(long long step){
  ofstream file;
 ...
 // Sending a number as a stream into output
  ...
  string savefile;
  savefile = outputname + ".sXY" + step_strg;
  file.open(savefile.c_str());
  file.precision(15);
  ...
  for(int j=0;j<ncells;j++){
    ...
    int fy = (j % (mx*my)) / mx;
    int fz = j / (mx*my);
    if (fy == 1) //close to the wall
      {
	counter = counter + 1;
	 file << fx << " " << fz << " " << csXY[j] << endl;	
      }
...
/******** Added by Adolfo **********/
bool saveFluid_scratch(long long step){
  ofstream file;
 ...
 // Sending a number as a stream into output
  ...
  string savefile;
  savefile = outputname + ".scratch" + step_strg;
  file.open(savefile.c_str());
  file.precision(15);
  ...
  for(int j=0;j<ncells;j++)
    if(particlesWall){
      ...
      fy++;
      i = fx + fy*mxt + fz*mxt*myt;
      // file << fy  << " " << cvx[i] << " " << cvy[i] << " " << cvz[i] << endl;
      file << fy  << " " << cvx[i] << " " << cvy[i] << " " << cvz[i] << " " << csXY[i] << endl;
    }
