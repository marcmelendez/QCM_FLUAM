...
bool saveFunctionsSchemeParticlesWall(int index, long long step){
  ...
  /**** Comment by Adolfo ****/
  /* I think that index=0 is for the initialization,
     index=1 is for the running simulation
     and index=2 is for the end of simulation */
  /***************************/
  if(index==0){
  ...
  //Use save functions
    ...
    /***** Added by Adolfo *****/
    // if(!saveFluid(index, step)) return 0;
    // if(!saveFluid_sXY(step)) return 0;
    // if(!saveFluid_scratch(step)) return 0;
  }
