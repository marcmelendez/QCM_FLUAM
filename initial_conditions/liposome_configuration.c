/* Generate configurations of a liposome tethered to a wall by a DNA strand */
...
int main(int argc, char * argv[])
{
  /* Usage message */
  if(argc < 8) {
    printf("Usage: %s <number of configurations> <number of monomers> <persistence length> <liposome radius> <linker energy> <monomer colour> <liposome colour>\n\n"
  ...
  int monomerc = atoi(argv[6]); /* Colour (type) of the monomers */
  int liposomec = atoi(argv[7]); /* Colour (type) of the lipsomes */
  double conf[3*(nmonomers + 1)]; /* Configuration of strand + liposome */
  double lp = atof(argv[3]); /* Persistence length */
  double gamma = 1.0/lp; /* Friction coefficient */
  /* Assume kT = M = 1 */
  double R = atof(argv[4]); /* Liposome radius (measured in monomer diameters) */
  float linker_energy = atof(argv[5]); /* Energy of angular spring linking DNA strand to wall */
  double dt = 0.01; /* Time step */
  double rij[3]; /* Displacement vector */
  double r2; /* Distance squared */
  double raxis[3]; /* Random axis */
  ...
  for(n = 0; n < nconfigurations; n++) {
    ...
    while(i < nmonomers) {
      ...
      if(i == nmonomers) {
        ...
        random_axis(raxis);
        for(k = 0; k < 3; k++) conf[3*i + k] = conf[3*(i - 1) + k] + R*raxis[k];
    ...
    /* Output result (all spheres) */
    for(i = 0; i < nmonomers + 1; i++)
      printf("%f\t%f\t%f\t%f\t%d\n", conf[3*i], conf[3*i + 1], conf[3*i + 2], (i == nmonomers)? R : .5, (i == nmonomers)? liposomec : monomerc);
    printf("\n");
  }
