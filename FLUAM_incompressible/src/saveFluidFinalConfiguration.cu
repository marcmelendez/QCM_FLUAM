...
bool saveFluid_gradv_average(long long step)
{
 ...
 // Sending a number as a stream into output
  ...
  string savefile;
  savefile = outputname + ".gradv";

  if(step == 0)
    file.open(savefile.c_str(), std::fstream::out | std::fstream::trunc);
  else
    file.open(savefile.c_str(), std::fstream::out | std::fstream::app);
  file.precision(15);
  ...
  for(int j=0;j<ncells;j++){
    ...
    int fz = j / (mx*my);
    double invdz = double(mz)/lz;
    if (fz == mz/2+1) //close to the wall (but not exactly on it)
      {
				counter = counter + 1;
				int fx = j % mx; // x coordinate of cell
				int fy = (j % (my*mx)) / mx; // y coordinate of cell
...
# ifdef CUBIC_GRADV
				...
				/*** We look for a polynomia v(h) = A3 h^3 + A2 h^2 + A1 h + A0.
	 			...
	 			The velocity gradient at h = 0 is A1 * invdz     *****/
  			...
  			double A0 = 1.0/2.0 * (1.0/3.0 * (fz*fz*fz + 6.0*fz*fz + 11.0*fz + 6.0)*cvx[j] +fz*(cvx[vec2]*(fz*fz+4.0*fz +3.0) - 1.0/3.0*cvx[vec3]*(fz*fz + 3.0*fz + 2.0) - cvx[vec1]*(fz*fz+5.0*fz+6.0)));
	 			...
	 			gradv  += (3.0*A3*zcoord*zcoord + 2.0*A2*zcoord + A1) * invdz;
        v += A3 * pow(zcoord, 3) + A2 * zcoord*zcoord + A1 * zcoord + A0;
# else

	 			double A1 = (cvx[vec1] - cvx[j])*invdz;
  			double A0 = cvx[j] - A1*fz;
...
# endif
  ...
  /* Output time step, du(z, t)/dt,  u(z, t), with u(z, t) representing the velocity field */
  file << step << " " << gradv/counter << " " << v/counter << endl;
